...
const resetForm = () => {
  coverage_area_form.value?.reset();
};
    ...
    <div @click="is_showing_modal = true">
      <slot name="trigger-button">
        <VBtn size="40" prepend-icon="tabler-plus">
          <VTooltip activator="parent"> Tambah Cakupan Area </VTooltip>
        </VBtn>
      </slot>
    </div>
    <VDialog :model-value="is_showing_modal" max-width="600" persistent>
      ...
      <VCard>
        <VCardItem>
          ...
          <template #title> Tambah Cakupan Area </template>
        </VCardItem>
        <VCardText>
          <VForm ref="coverage_area_form" @submit.prevent="saveCoverageArea">
            <VRow>
              <!-- NAME -->
              <VCol cols="12">
                <VTextField
