...
const pay_off_fee_data = ref({
  ...
  pay_off_nominal: 0,
  description: `Pembayaran Bonus Mitra (${props?.data?.name})`,
});

// FUNCTION
const payOffMitraFee = () => {
  pay_off_fee_form.value?.validate().then(({ valid: is_valid }) => {
    if (is_valid) {
      is_on_process.value = true;
      const params = {
        id_user: pay_off_fee_data.value._id,
        date: pay_off_fee_data.value.date,
        nominal: pay_off_fee_data.value.pay_off_nominal,
