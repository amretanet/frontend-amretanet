...
const change_submission_data = ref({
  type: null,
  id_package: null,
  id_customer: store.getUser.id_customer,
  reason_message: null,
});
...
const saveChangeSubmission = () => {
  change_submission_form.value?.validate().then(({ valid: is_valid }) => {
    if (is_valid) {
      is_on_process.value = true;
      axiosIns
        .post("change-submission/add", {
          data: change_submission_data.value,
        })
        .then(() => {
          showActionResult(undefined, undefined, "Pengajuan Telah Dikirimkan!");
          emits("changeSubmissionAdded");
          resetForm();
        })
        .catch((err) => {
...
const resetForm = () => {
  change_submission_form.value?.reset();
  change_submission_data.value.id_customer = store.getUser.id_customer;
};
    ...
    <div @click="is_showing_modal = true">
      <slot name="trigger-button">
        <VBtn size="40" prepend-icon="tabler-plus">
          <VTooltip activator="parent"> Tambah Pengajuan Perubahan </VTooltip>
        </VBtn>
      </slot>
    </div>
    <VDialog :model-value="is_showing_modal" max-width="500" persistent>
      ...
      <VCard>
        <VCardItem>
          ...
          <template #title> Tambah Pengajuan Perubahan </template>
        </VCardItem>
        <VCardText>
          <VForm
            ref="change_submission_form"
            @submit.prevent="saveChangeSubmission"
          >
            <VRow>
              <!-- TYPE -->
              <VCol cols="12">
